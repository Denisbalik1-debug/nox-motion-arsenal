...
const PRESETS: Record<LogoMarqueeVariant, LogoMarqueeItem[]> = {
  clients: [
    { name: 'NOX Labs', mark: 'NOX' },
    { name: 'Ulmer Gusto', mark: 'UG' },
    { name: 'Regalprüfung Süd', mark: 'RS' },
    { name: 'Sailer Systems', mark: 'SS' },
    { name: 'Local Growth', mark: 'LG' },
    { name: 'Agent Stack', mark: 'AS' },
  ],
  partners: [
    { name: 'Automation', mark: 'AUT' },
    { name: 'AI Systems', mark: 'AI' },
    { name: 'Revenue Ops', mark: 'REV' },
    { name: 'Creative Tech', mark: 'CT' },
    { name: 'Data Layer', mark: 'DATA' },
    { name: 'Growth Engine', mark: 'GE' },
  ],
  press: [
    { name: 'Featured Work', mark: 'FW' },
    { name: 'Digital Craft', mark: 'DC' },
    { name: 'Future Commerce', mark: 'FC' },
    { name: 'Local Leaders', mark: 'LL' },
    { name: 'AI Builders', mark: 'AIB' },
    { name: 'Modern Business', mark: 'MB' },
  ],
};

export function NoxLogoMarquee({
  variant = 'clients',
  speed = 28,
  direction = 'left',
  pauseOnHover = true,
  ...
  return (
    <div className={`nlm-stage ${fadeEdges ? 'with-fade' : ''}`}>
      <style>{`
        ...
        .nlm-name{font-size:14px;font-weight:700;letter-spacing:-.01em;white-space:nowrap}
        .nlm-stage.with-fade::before,.nlm-stage.with-fade::after{content:'';position:absolute;top:0;bottom:0;width:min(14vw,150px);z-index:3;pointer-events:none}
        .nlm-stage.with-fade::before{left:0;background:linear-gradient(90deg,#070708,transparent)}
        .nlm-stage.with-fade::after{right:0;background:linear-gradient(-90deg,#070708,transparent)}
        @keyframes nlm-scroll{to{transform:translateX(calc(-50% - 7px))}}
        @media(max-width:620px){.nlm-shell{gap:18px}.nlm-item{min-width:158px;padding:13px 15px}.nlm-mark{width:36px;height:36px}.nlm-name{font-size:12px}}
        @media(prefers-reduced-motion:reduce){.nlm-track{animation:none;transform:none;flex-wrap:wrap;width:auto;justify-content:center;padding-inline:18px}.nlm-stage.with-fade::before,.nlm-stage.with-fade::after{display:none}}
      `}</style>
      <div className={`nlm-shell ${pauseOnHover ? 'pause' : ''}`}>
        <div className="nlm-kicker">Selected clients · partners · references</div>
        <div
          className={`nlm-track ${direction === 'right' ? 'reverse' : ''}`}
          style={{ '--nlm-duration': `${duration}s` } as React.CSSProperties}
          aria-label={`${variant} logo marquee`}
        >
          {repeated.map((item, index) => {
            ...
            return item.href ? (
              <a key={`${item.name}-${index}`} className="nlm-item" href={item.href} target="_blank" rel="noreferrer">{content}</a>
            ) : (
              <div key={`${item.name}-${index}`} className="nlm-item">{content}</div>
            );
          })}
        </div>
      </div>
