...
const STYLES = `
.ngc-root{--ngc-accent:122,126,255;--ngc-secondary:45,226,210;--ngc-light:238,249,255;--ngc-glow:1;--ngc-depth:1;--ngc-opacity:.86;}
...
export function NoxGlassCards({
  ...
  useShaderQuad(canvasRef, {
    fragment: FRAGMENT,
    running: !reduced && activeCard !== null,
    frozenTime: 4,
    pointerRef: pointer,
    dprCap: 1.5,
    uniforms: {
  ...
  return (
    <div ref={rootRef} className="ngc-root ngc-field" data-variant={activeVariant} data-energy={activeEnergy} style={style}>
      <style>{STYLES}</style>
      ...
      <div className="ngc-stage">
        <div className="ngc-cards">
          {preset.cards.map((card, i) => (
            <div key={`${activeVariant}-${card.tag}`} ref={(el: HTMLDivElement | null) => { cardRefs.current[i] = el; }} className="ngc-card" onPointerMove={onMove(i)} onPointerEnter={() => !reduced && setActiveCard(i)} onPointerLeave={onLeave(i)} onPointerDown={onImpact(i)}>
              <div className="ngc-depth" />
              <div className="ngc-glass-slot" />
              <div className="ngc-fallback" />
              <div className="ngc-caustic" />
              <div className="ngc-rim" />
              <div ref={(el: HTMLDivElement | null) => { impactRefs.current[i] = el; }} className="ngc-impact" />
