...
export function GrainFilmOverlay({
  ...
  const style = {
    ...
    '--gfo-tile': `${clamp(grainScale, 60, 400)}px`,
    '--gfo-glow': glowColor,
    '--gfo-noise': `url("${NOISE_TILE}")`,
  } as React.CSSProperties;

  return (
    <div className="nox-gfo" style={style}>
      <style>{CSS}</style>
      ...
      {glow && (
        <>
          <span className="nox-gfo__blob nox-gfo__blob--a" aria-hidden="true" />
          <span className="nox-gfo__blob nox-gfo__blob--b" aria-hidden="true" />
        </>
      )}
