...
export function BlurryTextReveal({
  ...
  }, [reduced, text, intensity, speed, stagger]);
      ...
      <h2 className="nox-btr__headline">
        {lines.map((line, index) => (
          <span
            ...
            style={{ '--btr-delay': `${(index * step).toFixed(3)}s` } as React.CSSProperties}
          >
            {line}
          </span>
        ))}
...
.nox-btr__line { display:block; filter:blur(var(--btr-blur)); opacity:0; transform:translateY(20%); transition:filter var(--btr-dur) cubic-bezier(.22,1,.36,1) var(--btr-delay), opacity var(--btr-dur) ease var(--btr-delay), transform var(--btr-dur) cubic-bezier(.22,1,.36,1) var(--btr-delay); will-change:filter,transform; }
