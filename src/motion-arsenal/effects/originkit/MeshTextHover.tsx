...
/**
 * MeshTextHover — WebGL2 mesh-deformation text. A grid of vertices is
 * displaced by cursor drag (spring-damped), with an optional chromatic
 * RGB-split fringe on the displaced edges.
 * Ported from Originkit (originkit.dev/components/meshtexthover), Framer
 * runtime stripped.
 */
...
export default function MeshTextHover(props: Props) {
  const {
    text = 'MESH TEXT',
    color = '#ffffff',
    fontFamily = 'Inter',
  ...
  customColorsRef.current = [parseColor(colorA), parseColor(colorB)];
  ...
  useEffect(() => {
    ...
    const rebuildTex = async () => {
      ...
      try {
        const fontStr = `${fontWeight} ${realSize}px ${fontFamily}`;
      ...
      if (cancelled) return;
      const c2 = renderTextToCanvas(text, color, fontFamily, fontWeight, 'normal', realSize, w, h);
      gl.bindTexture(gl.TEXTURE_2D, tex);
    ...
    const tick = () => {
      ...
      const cA = cols[0] ?? [1, 0, 0];
      const cB = cols[1] ?? [0, 0, 1];
  ...
  }, [text, color, fontFamily, fontWeight, fontSize]);
  ...
  return (
    ...
    >
      <canvas ref={canvasRef} style={{ display: 'block', width: '100%', height: '100%', ...(style || {}) }} />
    </div>
  );
}
