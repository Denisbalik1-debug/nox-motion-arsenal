...
//   overrides={{"imageWidth":250,"imageHeight":250,"backgroundColor":"rgba(255, 255, 255, 0)"}}
//   __curationVersion={1}
...
export default function KlarnaCarousel(props: KlarnaCarouselProps) {
    const {
        ...
        labelY = 0,
        labelColor = "#ffffff",
        labelFont = { fontFamily: "Inter", fontSize: 26, lineHeight: "1.3em" },
    ...
    // pos is the single source of truth — a continuous float.
    // Math.round(pos) gives the active item index (mod M).
    // All button slots derive from (itemIdx - posDisplay), nothing else.
    ...
    // Show exactly `buttonCount` buttons (3/5/7), capped by the item count.
    ...
    // The end buttons ease off: full opacity up to (half - 0.4) slots, then a
    // gentle fade that reaches 0 a little past the window edge. This softens the
    // outermost buttons and hides the ring's wrap-around seam.
    ...
    const select = useCallback(
        (itemIdx: number) => {
            ...
            // Compute shortest signed delta from current pos — handles rapid clicks correctly
            // because we use posRef.current (live float), not stale `active` state
            ...
            // Start from wherever pos currently is (mid-animation if clicking fast)
            animRef.current = {
    ...
    // Render buffer: half+1 items each side of current rounded pos. Dedupe so a
    // small item count can't yield the same index twice (duplicate React keys →
    // buttons rendering at random/overlapping positions).
    ...
    function getVisualSlot(itemIdx: number): number {
        // Derive slot purely from posDisplay — the single animated float
    ...
    function slotStyle(slot: number) {
        ...
        // Full opacity within (half - 0.4) slots, then a gentle fade to 0 by
        // fadeEnd — softens the end buttons and hides the wrap-around seam.
        const opacity =
            absSlot <= fadeInner
                ? 1
                : absSlot >= fadeEnd
                    ...
                    return (
                        ...
                        >
                            <div
                                ...
                                onClick={
                                    isStatic ? undefined : () => select(itemIdx)
                                }
                            >
