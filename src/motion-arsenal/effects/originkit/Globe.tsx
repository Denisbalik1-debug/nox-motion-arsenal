...
/**
 * Globe — interactive 3D rotating globe (dot or solid landmass fill, geo
 * markers, grid lines, drag/auto-rotate). Ported from Originkit
 * (originkit.dev/components/globe): CDN three.js/d3-geo imports replaced
 * with real npm packages, Framer RenderTarget/ComponentMessage stripped.
 */
...
export default function Globe({
  ...
  useEffect(() => {
    ...
    let dotInstances: Mesh | InstancedMesh | null = null;
    ...
    const loadWorldData = async () => {
      try {
        ...
        while (continentOutlineGroup.children.length > 0) continentOutlineGroup.remove(continentOutlineGroup.children[0]);
        ...
        const pathGenerator = geoPath(projection, ctx as any);
        ctx.fillStyle = '#000';
    ...
    const animate = () => {
      ...
      if (needsContinue) animationFrameId = requestAnimationFrame(animate);
  ...
  const containerStyle: CSSProperties = { ...style, position: 'relative', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' };

  if (error) {
    return (
      <div style={containerStyle}>
        <div style={{ color: 'var(--text-dim, #999)', fontSize: 12.5, fontFamily: 'monospace', textAlign: 'center' }}>{error}</div>
      </div>
    );
  }
