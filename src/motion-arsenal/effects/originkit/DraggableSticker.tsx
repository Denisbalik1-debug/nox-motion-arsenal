...
/**
 * DraggableSticker — drag a WebGL "photo sticker" that peels off the surface
 * and settles back with sheen/holo lighting. Ported from Originkit
 * (originkit.dev/components/draggablesticker), Framer runtime + zoom-probe
 * canvas-editor branch stripped (plain ResizeObserver only).
 */
...
export default function DraggableSticker({
  ...
  const [sizingDimensions, setSizingDimensions] = useState({ width: 400, height: 400 });
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', height: '100%', overflow: 'visible', perspective: '800px', ...style }}>
      <div
        style={{
          position: 'relative',
      ...
      >
        <div ref={innerRef} style={{ position: 'absolute', width: '100%', height: '100%', transformStyle: 'preserve-3d', overflow: 'visible' }}>
          <canvas ref={canvasRef} style={{ display: 'block', pointerEvents: 'none', position: 'absolute' }} />
        </div>
      </div>
    </div>
  );
