...
/**
 * FluidTrail — GPU fluid simulation (Navier-Stokes-lite via WebGL2) that
 * swirls behind the cursor. Ported from Originkit (originkit.dev/components/
 * fluidtrail), Framer runtime stripped.
 */
...
function createFBO(gl: WebGL2RenderingContext, w: number, h: number, internalFormat: number, format: number, type: number, filter: number) {
...
function createDoubleFBO(gl: WebGL2RenderingContext, w: number, h: number, internalFormat: number, format: number, type: number, filter: number) {
...
export default function FluidTrail(props: Props) {
  ...
  useEffect(() => {
    ...
    function applyPointerInput() {
      ...
      const col: [number, number, number] = [c[0] * 0.7, c[1] * 0.7, c[2] * 0.7];
  ...
  return (
    <canvas
      ref={canvasRef}
      style={{ display: 'block', width: '100%', height: '100%', touchAction: 'none', ...(props.style || {}) }}
    />
  );
}

FluidTrail.defaultProps = {
  mouseRadius: 7,
  trailDuration: 30,
  color: '#66FF9C',
  fade: 'outside',
