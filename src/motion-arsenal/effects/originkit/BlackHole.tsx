...
/**
 * BlackHole — 3D black hole accretion disk. Particles orbit a gravity well,
 * z-sorted so they pass believably behind/in front of the event horizon.
 * Ported from Originkit (originkit.dev/components/blackhole), Framer runtime
 * stripped (RenderTarget checks, property-controls → plain props).
 */
...
export default function BlackHole({
  ...
  const initParticles = useCallback((count: number, horizonRad: number, outerRad: number, colorsLength: number) => {
    const pts: Particle[] = [];
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      type ProjectedPt = { x: number; y: number; size: number; alpha: number; z: number; color: string };
  ...
  }, [voidX, voidY, voidRadius, voidColor, showCenter, particleCount, particleSize, JSON.stringify(colors), outerRadFromSize, tilt, tiltSideway, trailAlpha, orbitSpeed, pullSpeed, perspective]);

  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', background: '#000000', ...style, position: 'relative', overflow: 'hidden' }}>
      <canvas ref={canvasRef} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }} />
      <canvas ref={fgCanvasRef} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none' }} />
    </div>
  );
}
